...
08 - Get All Packages Test
    ...
    ...    ELSE IF      ${has_records}   Log    ${packages['records'].__len__()} packages
...
10 - Delete Package Test
    ...
    Run Keyword If      ${response.status_code} != 200    Log    Package successfully deleted
    ...    ELSE    Fail    Package was not deleted
...
12 - Bulk Update Packages Test
    [Documentation]     Test bulk updating multiple packages
    [Tags]              update    bulk
    
    # Create multiple packages first
    ${package1_data}=   Generate Unique Package Data
    ${package1_id}    ${package1_response}=    Create Package    ${package1_data}
    
    ${package2_data}=   Generate Unique Package Data
    ${package2_id}    ${package2_response}=    Create Package    ${package2_data}
    
    # Prepare bulk update data
    ${bulk_update_data}=    Create Dictionary
    ...                 ids=${package1_id},${package2_id}
    ...                 status=SHIPPING
    ...
    ${json}=            Evaluate         json.loads('''${response.text}''')    json
    Should Not Be Empty    ${json}
    ...    msg=Bulk update packages response was empty
    
    Log                 Successfully bulk updated packages: ${package1_id}, ${package2_id}
...
13 - Cleanup Test Environment
    ...
    Log                 Test environment cleaned up successfully04 - Get Package Test
