...
${REPLENISHMENT_ID}     ${EMPTY}
${PRODUCT_ORDER_ID}     ${EMPTY}
${PACKAGE_ID}           ${EMPTY}
...
Generate Receipt Data
    [Documentation]     Generate unique receipt data with products
...
Generate Product Data
    ...
    [Arguments]         ${receipt_id}    ${warehouse_id}    ${master_product_id}
    ${timestamp}=       Evaluate         int(time.time())    time
...
Generate Product Order Data
    [Documentation]     Generate product order data with product constraint
...
05 - Create Receipt Test
    [Documentation]     Test creating a receipt with products
    [Tags]              create    receipts    products    manager
    ${receipt_data}=    Generate Receipt Data    ${WAREHOUSE_ID}
    ${product_data}=    Generate Product Data    ${RECEIPT_ID}    ${WAREHOUSE_ID}    ${MASTER_PRODUCT_ID}
    ...
    Set Global Variable    ${PRODUCT_ID}    ${response}[products][0][id]
...
07 - Create Product Order Test
    [Documentation]     Test creating product order with product constraint
    [Tags]              create    product-orders    manager
    ${order_data}=      Generate Product Order Data    ${PRODUCT_ID}
