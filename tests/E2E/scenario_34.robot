...
...               Includes positive, negative, edge, dependency, bulk, concurrency, stress, integrity, and workflow tests
...
Teardown Test Suite
    ...
    Log                 Suite completed in ${duration} seconds with ${OPERATION_COUNT} API operations
...
Generate Unique Warehouse Data
    ...
    [Arguments]         ${acreage}=${SMALL_ACREAGE}
...
Create Warehouse With Retry
    ...
    [Arguments]         ${warehouse_data}    ${timeout}=${FALSE}
...
04 - Create Warehouse Max Acreage Test
    ...
    Log Response Details  ${response}    Warehouse Creation (Max Acreage)
...
11 - Create Product Max Quantity Test
    ...
    Log Response Details  ${response}    Product Creation (Max Quantity)
...
27 - Test Warehouse State Persistence
    ...
    Set Global Variable  ${TEST_WAREHOUSE_DATA}    ${update_data}
    Log Response Details  ${updated_warehouse}    Warehouse Update (Persistence)
...
28 - Test Warehouse Creation Timeout
    ...
    Delete Warehouse    ${warehouse_id}
    Log Response Details  ${response[1][1]}    Warehouse Creation (Timeout)
...
29 - Test Full Workflow
    ...
    Should Not Be Empty    ${warehouse_id}
    Log Response Details  ${w_response}    Workflow Warehouse Creation
...
30 - Create Order Before Product Test
    ...
    ...                 expected_status=anything
    Should Not Equal    ${response.status_code}    201
    Log Response Details  ${response}    Order Creation (Invalid Product)
...
32 - Delete Warehouse With Dependencies Test
    [Documentation]     Test deleting a warehouse with linked supplier fails or cascades
    [Tags]              delete    negative    integrity
    ${headers}=         Create Dictionary    Content-Type=application/json    Authorization=${AUTH_TOKEN}
    ${response}=        DELETE On Session
    ...                 vkho
    ...                 /warehouses/delete/${TEST_WAREHOUSE_ID}
    ...                 headers=${headers}
    ...                 expected_status=anything
    Run Keyword If      ${response.status_code} == 200
    ...                 Log    Warning: Warehouse deletion succeeded despite dependencies; cascade behavior detected
    ...                 ELSE
    ...                 Log    Successfully verified that warehouse deletion fails with dependencies
    Log Response Details  ${response}    Warehouse Deletion (With Dependencies)

33 - Delete Order Test
    [Documentation]     Test deleting the updated order
    [Tags]              delete    positive
    ${result}=          Delete Order    ${TEST_ORDER_ID}
    Should Be True      ${result}
    Set Global Variable  ${TEST_ORDER_ID}    ${EMPTY}
    Log                 Successfully deleted order with ID: ${TEST_ORDER_ID}

34 - Delete Product Test
    [Documentation]     Test deleting the updated product
    [Tags]              delete    positive
    ${result}=          Delete Product    ${TEST_PRODUCT_ID}
    Should Be True      ${result}
    Set Global Variable  ${TEST_PRODUCT_ID}    ${EMPTY}
    Log                 Successfully deleted product with ID: ${TEST_PRODUCT_ID}

35 - Delete Supplier Test
    [Documentation]     Test deleting the updated supplier
    [Tags]              delete    positive
    ${result}=          Delete Supplier    ${TEST_SUPPLIER_ID}
    Should Be True      ${result}
    Set Global Variable  ${TEST_SUPPLIER_ID}    ${EMPTY}
    Log                 Successfully deleted supplier with ID: ${TEST_SUPPLIER_ID}

36 - Delete Warehouse Test
    [Documentation]     Test deleting the updated warehouse after dependencies are cleared
    [Tags]              delete    positive
    ${result}=          Delete Warehouse    ${TEST_WAREHOUSE_ID}
    Should Be True      ${result}
    Set Global Variable  ${TEST_WAREHOUSE_ID}    ${EMPTY}
    Log                 Successfully deleted warehouse with ID: ${TEST_WAREHOUSE_ID}

37 - Delete Non-Existent Warehouse Test
...
38 - Create Multiple Warehouses Test
...
39 - Simulate Concurrent Product Updates Test
...
40 - Stress Test Multiple Products Creation
...
41 - Test Idempotent Warehouse Update
