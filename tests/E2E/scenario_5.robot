*** Settings ***
Documentation     End-to-End Test Suite for Order Creation to Cancellation in vKho API
...               Covers order creation, picking, packaging, and cancellation
...
Generate Unique Order Data
    ...
    ...                 code=${order_code}
    ...                 boothCode=BOOTH${timestamp}
    ...                 deliveryAdress=456 Cancel St
    ...                 deliveryTime=2025-04-02T14:00:00.000Z
    ...                 driverName=Jane Doe
...
Check Picking Order
    [Documentation]     Check if order can proceed to picking
    [Arguments]         ${order_id}    ${user_id}=test-user
    
    Should Not Be Empty    ${order_id}
    
    ${check_data}=      Create Dictionary    ids=${[ ${order_id} ]}    userId=${user_id}
    ${headers}=         Create Dictionary    Content-Type=application/json    Authorization=${AUTH_TOKEN}
...
02 - Create Order Test
    ...
    ${order_data}=      Generate Unique Order Data    Cancel Order
    ${order_id}         ${response}=    Create Order    ${order_data}
...
05 - Check Picking Order Test
    [Documentation]     Test checking picking availability
    [Tags]              picking    positive
    
    Run Keyword If      "${TEST_ORDER_ID}" == "${EMPTY}"    Create Test Order
    
    ${result}=          Check Picking Order    ${TEST_ORDER_ID}
    Should Be True      ${result}
    
    Log                 Successfully checked picking availability for order: ${TEST_ORDER_ID}
...
07 - Update Order to Cancelled Test
...
08 - Verify Cancelled Order Test
    [Documentation]     Test retrieving the cancelled order
    [Tags]              retrieve    positive
    ...
    Log                 Successfully verified order as CANCELLED: ${TEST_ORDER_ID}
